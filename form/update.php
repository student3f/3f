<?php
   ...
   function getB(){
     if(isset($_GET['v'])){
       $bn = basename($_GET['v']);
       echo file_get_contents('folder/'.$_GET['vw'].'/'.$bn);
     }
   }
  ?>
</div>
...
<div id="m">
  <form id="form" action="u2.php" method="post" enctype="multipart/form-data">
  <div id="tb" style="text-align: center;">
  <p><input type="text" placeholder="title" name="n" id="ti" value="<?= getT(); ?>"></p>
 </div>
 <div id="bt">
   <p><input type="hidden" name="o" value="<?= $_GET['v'] ?>"></p>
   <p><input type="hidden" name="le" value="<?= $_GET['vw'] ?>"></p>
   <p><textarea placeholder="description" name="de" id="ta" rows="30" cols="30"><?= getB(); ?></textarea></p>
